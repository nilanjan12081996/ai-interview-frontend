...
import { Code, Lightbulb, ClipboardList, Timer, Cpu, BookOpen, AlertCircle, Loader2 } from "lucide-react";

const CodingReportPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          let questionRes = await axios.get(`${baseUrl}/api/coding/get-question-pdf/${token}`);

          let qData = questionRes.data?.data || questionRes.data;

          if (qData?.codingDTO?.questionData) {
            setCodingData(qData.codingDTO.questionData);
          } else if (qData?.questionData) {
            setCodingData(qData.questionData);
          } else {
            setCodingData(qData);
          }
        } catch (err) {
        ...
        try {
          let ansRes;
          try {
            ansRes = await axios.get(`${baseUrl}/api/aiinterview/coding/ans/code-ans/${token}`);
          } catch (e) {
            ansRes = await axios.get(`${baseUrl}/code-ans/${token}`);
          }

          let aData = ansRes.data?.data || ansRes.data;

          if (aData?.ans?.ans) {
            setCodingAnswersData(aData.ans.ans);
          } else if (aData?.ans) {
            setCodingAnswersData(aData.ans);
          } else if (aData?.answers) {
            setCodingAnswersData(aData.answers);
          } else {
            setCodingAnswersData(aData);
          }
        } catch (err) {
        ...
        console.error(err);
        setError("Failed to fetch coding assessment data");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error || (!codingData && !codingAnswersData)) {
    return (
      <div className="flex min-h-screen w-full flex-col items-center justify-center bg-slate-50 text-slate-500">
        <div className="p-6 bg-white rounded-full mb-4 shadow-sm border border-slate-100">
          <Code size={48} className="text-slate-300" />
        </div>
        <p className="font-bold text-xl text-slate-700">No Assessment Data Found</p>
        <p className="text-sm mt-2">{error || "There are no coding questions available for this candidate."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-10 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-[1000px] bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
        <div className="p-6 bg-slate-50/80 backdrop-blur-md border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-violet-100 rounded-xl">
              <Code className="text-violet-600 w-6 h-6" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-slate-800">Coding Assessment Report</h2>
                ...
                <TabsContent key={idx} value={`q-${idx}`} className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-slate-50 pb-8">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className={`${q.difficulty === 'ADVANCE' || q.difficulty === 'HARD' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                          ...
                          {answers[idx] && answers[idx].timeData && (
                            <div className="flex flex-wrap items-center gap-3">
                              ...
                              {answers[idx].evaluation && (
                                <>
                                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${answers[idx].evaluation.status === 'PASSED' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'
                                    }`}>
...
export default CodingReportPage;
