...
//   let userTokenData;
//   try {
//     userTokenData = JSON.parse(sessionStorage.getItem('ai_interview_token'));
...
api.interceptors.request.use((req) => {
  ...
  // ✅ 1. Check if the payload is FormData
  if (req.data instanceof FormData) {
    // Delete the header entirely! The browser will automatically add it back 
    // WITH the correct 'boundary=...' string attached.
    delete req.headers['Content-Type'];
  } else {
    // ✅ 2. Otherwise, safely default to JSON
    req.headers['Content-Type'] = 'application/json';
  }
